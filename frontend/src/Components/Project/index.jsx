...
const Project = () => {
  ...
  const handleFilter = (selectedFilter) => {
    ...
    const list = projectList.filter((project) => project?.technology === selectedFilter)
    ...
    />
      ...
      {filteredList?.map( (project)=>
        ...
        <Col className='project-card' md={12}>
          <div className="card">
        <h3>{project?.name}</h3>
        <span>{project?.Description?.length < 140 ? project?.Description : truncate(project?.Description,140)}</span>
        <span className='tech'>Technology: <span className={`${themeDecider()}`}>{project?.technology}</span></span>
        <div className='footer-wrapper'>
        <Tooltip title={'Disabled due to new upgrades'}>
          <button disabled className={`disabled ${themeDecider()}`}>Know more</button>
        </Tooltip>
        {project?.award && <Tooltip title={'Awarded for this project'}>
          <TrophyOutlined className={`${themeDecider()}`} style={{fontSize:'24px'}}/>
          </Tooltip>
          }
        </div>
        </div>
